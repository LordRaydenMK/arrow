...
obs
// ObservableK(observable=io.reactivex.internal.operators.observable.ObservableFromArray@634f0922)
```

```kotlin
val flow = Flowable.fromArray(1, 2, 3, 4, 5).k()
flow
// FlowableK(flowable=io.reactivex.internal.operators.flowable.FlowableFromArray@676afc8b)
```

```kotlin
val single = Single.fromCallable { 1 }.k()
single
// SingleK(single=io.reactivex.internal.operators.single.SingleFromCallable@5139198a)
```

```kotlin
val maybe = Maybe.fromCallable { 1 }.k()
maybe
// MaybeK(maybe=io.reactivex.internal.operators.maybe.MaybeFromCallable@4c7e34a6)
```

```kotlin
val subject = PublishSubject.create<Int>().k()
subject
// ObservableK(observable=io.reactivex.subjects.PublishSubject@443e8836)
```
...
```kotlin
obs.value()
// io.reactivex.internal.operators.observable.ObservableFromArray@634f0922
```

```kotlin
flow.value()
// io.reactivex.internal.operators.flowable.FlowableFromArray@676afc8b
```

```kotlin
single.value()
// io.reactivex.internal.operators.single.SingleFromCallable@5139198a
```

```kotlin
maybe.value()
// io.reactivex.internal.operators.maybe.MaybeFromCallable@4c7e34a6
```

```kotlin
subject.value()
// io.reactivex.subjects.PublishSubject@443e8836
```
...
| Module | Type classes |
|__arrow.effects.typeclasses__|[Async]({{ '/docs/arrow/effects/typeclasses/async' | relative_url }}), [ConcurrentEffect]({{ '/docs/arrow/effects/typeclasses/concurrenteffect' | relative_url }}), [Effect]({{ '/docs/arrow/effects/typeclasses/effect' | relative_url }}), [MonadDefer]({{ '/docs/arrow/effects/typeclasses/monaddefer' | relative_url }})|
